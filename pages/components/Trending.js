      ...
      {props.trendingCoins.map((coin, index) => {
        return (
            <div key={coin.item.name} className="min-w-[190px] hover:scale-105 transition duration-200 ease-out hover:text-black/100 bg-white font-semibold text-center rounded-3xl border shadow-lg p-10 max-w-xs m-20 border-gray-400">
                ...
                <button onClick={handleCoinInfo} className="hover:bg-red-300 bg-indigo-600 px-8 py-2 mt-8 rounded-3xl text-gray-100 font-semibold uppercase tracking-wide">More</button>
            </div>
           )
      })}
      <Modal show={showCoinInfo} onHide={handleCoinInfo}>
          <CoinInfo />
