import {GlobeIcon, AtSymbolIcon, SelectorIcon, GlobeAltIcon, CurrencyDollarIcon, DotsVerticalIcon} from '@heroicons/react/solid';
            ...
            <div className="flex flex-wrap md:flex-auto max-w-[100%] min-w-[1000px] justify-center">
                <div className="flex md:flex-wrap lg:inline-block md:flex ">
                    <div className=" bg-slate-100 min-h-[250px] max-h-[500px]  bd-gray hover:scale-105 transition duration-200 ease-out hover:text-black/100  font-semibold text-center rounded-3xl border shadow-lg p-10 max-w-xs m-20 border-gray-400">
                        ...
                        {props.country_origin?<p className="ml-3 text-left">{props.country_origin}</p>:<p className="ml-3 text-left">Origin Not Found</p>} 
                        </div>
                        <div className="flex m-3">
                            <AtSymbolIcon className="w-5 h-5"/>
                            <p className="ml-3 text-left "> {props.symbol}</p> 
                        </div>
                        
                        <div className="flex m-3">
                            <SelectorIcon className="w-5 h-5"/>
                            <p className="ml-3 text-left">Market cap rank: {props.market_cap_rank}</p>
                        </div>
                        <div className="flex m-3">
                            <CurrencyDollarIcon className="w-5 h-5"/>
                            <p className="ml-3 text-left">Price: ${props.price_usd}</p>
                        </div>
                        <div className="flex m-3">
                            <DotsVerticalIcon className="w-5 h-5"/>
                            <p className="ml-3 text-left">Circulating Supply: {props.circulating_supply}</p>
                        </div>

                        <div className="flex m-3">
                            <GlobeAltIcon className="w-5 h-5"/>
                            <a href={props.homepage}  className="ml-3 text-left underline underline-offset-8 decoration-sky-500 hover:bg-red-300">Website</a>
                        </div>
                    </div>
                    <div className=" min-h-[250px] max-h-[500px] bd-gray hover:scale-105 transition duration-200 ease-out hover:text-black/100 bg-slate-100 font-semibold text-center rounded-3xl border shadow-lg p-10 max-w-xs m-20 border-gray-400">
                        ...
                        <p className="mt-5">Public interest score: {props.public_interest_score}</p> 
